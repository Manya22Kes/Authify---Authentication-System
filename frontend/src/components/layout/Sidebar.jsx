...
const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 9a3.75 3.75 0 117.5 0c0 1.62-.93 2.47-2.03 3.23-.98.68-1.72 1.2-1.72 2.27m.03 3h.008v.008h-.008V17.5z" />
      </svg>
    ),
  },
  {
    label: 'Audit Logs',
    href: APP_ROUTES.AUDIT_LOGS,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function Sidebar({ onClose }) {
  ...
  return (
    <aside className="flex h-full w-[min(18rem,85vw)] flex-shrink-0 flex-col border-r border-obsidian-700/80 bg-obsidian-900">
      {/* Brand row */}
      <div className="px-5 py-5 border-b border-obsidian-800 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <span className="font-display font-bold text-ivory-100 tracking-tight truncate">Authify</span>
        </div>

        {/* Close button — only shown when rendered in the mobile drawer */}
        {onClose && (
          <button
            onClick={onClose}
            aria-label="Close navigation"
            className="lg:hidden flex-shrink-0 text-ivory-400/50 hover:text-ivory-100 transition-colors"
          >
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-0.5" aria-label="Main navigation">
        <p className="px-3 mb-2 text-xs font-mono text-sky-300/70 uppercase tracking-widest">Main</p>
        {navItems.map((item) => (
      ...
      <div className="border-t border-obsidian-800 p-3">
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-obsidian-800 transition-colors min-w-0">
...
function NavItem({ item }) {
  if (item.soon) {
    return (
      <div className="sidebar-item opacity-50 cursor-default" aria-disabled="true">
        {item.icon}
        <span className="flex-1 truncate">{item.label}</span>
        <span className="text-[10px] font-mono text-ivory-400/60 bg-obsidian-700 px-1.5 py-0.5 rounded flex-shrink-0">Soon</span>
      </div>
    );
  }
